    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-2xl max-h-[90vh] flex flex-col">
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b">
          <div>
            <h2 className="text-xl font-bold text-gray-800">Satın Alma Siparişi</h2>
            <p className="text-sm text-gray-500 font-mono mt-0.5">{po.po_number}</p>
          </div>
          <div className="flex items-center gap-3">
            <span className={`px-3 py-1 text-sm rounded-full font-medium ${STATUS_COLORS[po.status] || 'bg-gray-100 text-gray-700'}`}>
          ...
          <div className="grid grid-cols-2 gap-4">
            <div className="flex items-start gap-3 p-4 bg-gray-50 rounded-xl">
              ...
              <div>
                ...
                {po.supplier_contact && (
                  <p className="text-xs text-gray-500">{po.supplier_contact}</p>
                )}
              </div>
            </div>
            <div className="flex items-start gap-3 p-4 bg-gray-50 rounded-xl">
              ...
              <div>
                <p className="text-xs text-gray-500 mb-0.5">Beklenen Teslimat</p>
                <p className="text-sm font-semibold text-gray-800">{fmtDate(po.expected_delivery)}</p>
                {po.actual_delivery && (
            ...
            <div className="flex items-start gap-3 p-4 bg-gray-50 rounded-xl">
              ...
              <div>
                <p className="text-xs text-gray-500 mb-0.5">Oluşturulma</p>
            ...
            <div className="flex items-start gap-3 p-4 bg-green-50 rounded-xl">
              ...
              <div>
                ...
                {po.received_amount > 0 && (
                  <p className="text-xs text-gray-500">Alınan: {fmt(po.received_amount)}</p>
                )}
              </div>
            </div>
          </div>
                ...
                <table className="min-w-full divide-y divide-gray-200">
                  <thead className="bg-gray-50">
                    <tr>
                      ...
                      <th className="px-4 py-3 text-right text-xs font-medium text-gray-500 uppercase">Alınan</th>
                    ...
                    {po.items.map((item, idx) => (
                      <tr key={item.id || idx} className="hover:bg-gray-50">
                        <td className="px-4 py-3">
                          <p className="text-sm font-medium text-gray-800">{item.product_name || item.name || '—'}</p>
                        ...
                        <td className="px-4 py-3 text-right text-sm text-gray-700">{fmt(item.unit_price || item.price)}</td>
                  ...
                  <tfoot className="bg-gray-50 border-t-2 border-gray-200">
                    <tr>
                      <td colSpan={4} className="px-4 py-3 text-right text-sm font-semibold text-gray-700">Genel Toplam</td>
                      <td className="px-4 py-3 text-right text-base font-bold text-gray-900">{fmt(po.total_amount)}</td>
                    </tr>
                  </tfoot>
                </table>
              </div>
